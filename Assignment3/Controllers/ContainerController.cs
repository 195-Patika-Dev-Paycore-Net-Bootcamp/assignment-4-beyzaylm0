...
namespace Assignment3.Controllers
{
    ...
    public class ContainerController : ControllerBase
    {
        ...
        public ActionResult<List<List<Container>>> ContainerClusters(long vehicleId, int n)
        {

            var containerList = session.container.Where(v => v.vehicleid == vehicleId).ToList();
            double[][] rawData = new double[containerList.Count][];

            for (int i = 0; i < containerList.Count; i++)
            {
                rawData[i] = new double[] { containerList[i].latitude, containerList[i].longitude };
            ...
            return CreateNewList(clustering, numClusters, containerList);
        ...
        public static List<List<Container>> CreateNewList( int[] clustering, int numClusters,List<Container> container)
        {
            List<List<Container>> list = new List<List<Container>>();
            for(var i=0; i < numClusters; i++)
            {
                list.Add(new List<Container>());
            }

            for (int i = 0; i < clustering.Length; i++)
            {
                list[clustering[i]].Add(container[i]);
            }
            return list;
        ...
        {
            //List<int> containerClusters2 = new List<int>();
            Console.WriteLine(clustering.ToString());
            //foreach (var item in clustering)
            //{
            ...
            for (int k = 0; k < numClusters; ++k)
            {
                ...
                for (int i = 0; i < data.Length; ++i)
                {
                    ...
                    //containerClusters2.Add(clustering[i]);
                    for (int j = 0; j < data[i].Length; ++j)
                    {
                        if (data[i][j] >= 0.0) Console.Write(" ");
                        Console.Write(data[i][j].ToString("F" + decimals) + " ");
